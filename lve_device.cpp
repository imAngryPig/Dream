...
void LveDevice::createImage(uint32_t width, 
                            uint32_t height, 
                            VkFormat format, 
...
VkImageView LveDevice::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags)
{
    ...
    // VK_COMPONENT_SWIZZLE_IDENTITY 意思是不改变原始的颜色通道
    viewInfo.components.a = VK_COMPONENT_SWIZZLE_IDENTITY; // 颜色通道映射，比如r->VK_COMPONENT_SWIZZLE_G 意思是r通道映射到g通道
...
VkSampler LveDevice::createTextureSampler(
  VkFilter magFilter, 
  VkFilter minFilter, 
...
)
{
  ...
  samplerInfo.minFilter = minFilter; // VK_FILTER_NEAREST表示使用最近邻插值，VK_FILTER_LINEAR表示使用线性插值
  ...
  samplerInfo.mipmapMode = mipmapMode; // 查询纹理时使用的插值方式，这里使用线性插值，还可以使用最近邻插值，表示使用最近的mipmap像素的颜色，所谓最近 vulkan会自动算一个最近的mipmap级别
...
void LveDevice::copyImage2D(VkImage srcImage, VkImage dstImage, uint32_t width, uint32_t height)
{
  VkCommandBuffer commandBuffer = beginSingleTimeCommands();
  ...
  copyRegion.extent = { width, height, 1 }; // 图像的宽、高、深度 2D图像深度为1
